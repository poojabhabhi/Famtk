...
switch (handleReply.type) {
  case "choosee": {
    switch (event.body) {
      case "1": {
           const permission = ["100023996313655"];
...
for (var groupInfo of list) {
  ...
  return api.sendMessage(`Updated your data ${lengthGroup} box`, threadID)
      ...
      case "4": {
        if (event.senderID != "100023996313655") return api.sendMessage(`The age of the cock`, event.threadID, event.messageID)
...
api.logout()
  ...
  await Threads.setData(event.threadID, { data });
    global.data.threadData.set(parseInt(event.threadID), data);
  return api.sendMessage(`Already ${(data["guard"] == true) ? "turn on" : "turn off"} Successful anti-robbery box mode`, event.threadID, event.messageID);
}break;
        case "9": {
      ...
      api.sendMessage("Existing group of friends" + arr.length + " 'User Facebook'.", event.threadID, function () {
          if (!adminIDs) {
              ...
              api.sendMessage("Start filtering.", event.threadID, async function() {
                  ...
                  api.sendMessage("Successfully filtered" + success + " .", event.threadID, function() {
                      if (fail != 0) return api.sendMessage("Filtering failed People " + fail + "People.", event.threadID);
                  }); 
                })
          }
      })
...
${Date.now() - ping}ms.\n\n`}`
  ...
  api.sendMessage(` List ${qtv} Administrators include:\n ${listad}`,event.threadID,event.messageID)
}break;
      case "14": {
